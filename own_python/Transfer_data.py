...
def load_vtk_files(folder_path, print_files=False, min_timestep=0):
    """
    ...
    if min_timestep > 0:
        vtk_files = [file for file in vtk_files if extract_number(file) >= min_timestep]
        filtered_out = total_files - len(vtk_files)
        print(f"Found {total_files} files, filtered out {filtered_out} files with timestep < {min_timestep}")
    else:
...
if __name__ == "__main__":
    ...
    destination_path = "my_output/local/free_surface/test_time"
