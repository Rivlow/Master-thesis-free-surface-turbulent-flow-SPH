...
def main():
	...
	# Physical parameters
	r = (0.01/np.power(2, 1/2))*m                # Particle radius
	...
	#----Translation vectors----#
	x_init = 1.5*m
	trans_roof = [x_init + Lx_roof/2, Ly_foot + Ly_roof/2, 0]
	trans_mid_foot = [x_init + Lx_foot/2, Ly_foot/2, 0]
	trans_left_foot = [x_init + Lx_foot/2, Ly_foot/2, -Lz_foot/2 -offset/2]
	trans_right_foot = [x_init + Lx_foot/2, Ly_foot/2, +Lz_foot/2 +offset/2]

	trans_emit = [4*particle + Lx_emit/2, Ly_emit/2, 0]
	trans_anim = [Lx_dom - Lx_anim/2 - particle, Ly_anim/2, 0]

	y_init_wood = 2*water_height
	...
	placement_area_x_min = 0.5  
	placement_area_x_max = x_init/2 - 0.15
	placement_area_z_min = -Lz_dom/2 + 0.15
	placement_area_z_max = Lz_dom/2 - 0.15
	placement_area_y = y_init_wood 
	...
	FluidBlocks = [
		...
		{
			"denseMode": 0,
			"start": [8*particle, 0, -Lz_dom/2 + 2*particle],
			"end": [x_init, water_height, Lz_dom/2 - 2*particle],
			"initialVelocity": [U_0, 0.0, 0.0]
		},
		# After bridge
		{
			"denseMode": 0,
			"start": [x_init + Lx_roof, 0, -Lz_dom/2 + 2*particle],
			"end": [Lx_dom, water_height, Lz_dom/2 - 2*particle],
			"initialVelocity": [U_0, 0.0, 0.0]
		},
	]
	...
	Materials = [
		{
			...
			"emitterBoxMax": [Lx_dom-5*particle, 4*Ly_dom, Lz_dom/2]
		}
	]
	...
	'''# Inside bridge (inner left)
		{
			"denseMode": 0,
			"start": [x_init, 0, -Lz_foot - offset/2 + 2*particle],
			"end": [x_init+ Lx_roof, water_height, -Lz_foot/2 - particle],
			"initialVelocity": [0, 0.0, 0.0]
		},
		# Inside bridge (inner right)
		{
			"denseMode": 0,
			"start": [x_init, 0, Lz_foot/2 + particle],
			"end": [x_init+ Lx_roof, water_height, Lz_foot + offset/2 - 2*particle],
			"initialVelocity": [0, 0.0, 0.0]
		},
		# Inside bridge (outer left)
		{
			"denseMode": 0,
			"start": [x_init, 0, -Lz_dom/2 + particle],
			"end": [x_init+ Lx_roof, water_height, -offset/2 - Lz_foot - particle],
			"initialVelocity": [0, 0.0, 0.0]
		},
		# Inside bridge (outer right)
		{
			"denseMode": 0,
			"start": [x_init, 0, offset/2 + Lz_foot + particle],
			"end": [x_init+ Lx_roof, water_height, Lz_dom/2- particle],
			"initialVelocity": [0, 0.0, 0.0]
		},'''
